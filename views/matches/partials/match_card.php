...
<div class="col">
    <div class="card h-100 card-partita rounded-4 shadow-sm bg-body overflow-visible mt-3 position-relative d-flex flex-column" onclick="window.location.href='<?= url('/matches/' . $p['id']) ?>';" style="cursor: pointer;">
        ...
        <div class="card-body position-relative pt-4 mt-2 d-flex flex-column h-100">
            ...
            <div class="d-flex justify-content-between align-items-start mb-3 gap-1 flex-wrap">
                <div class="d-flex gap-1 align-items-center">
                    <span class="badge bg-body-secondary text-body border rounded-pill px-2 py-1 shadow-sm">
                        <i class="bi bi-calendar-event me-1 text-primary"></i> <?= e(date('d/m H:i', strtotime($p['date'] . ' ' . $p['time']))) ?>
                    </span>
                    <?php if(($p['visibility'] ?? 'public') === 'public'): ?>
                        <span class="badge bg-body-secondary text-secondary border rounded-pill px-2 py-1 shadow-sm" title="Partita Pubblica">
                            <i class="bi bi-globe"></i>
                        </span>
                    <?php else: ?>
                        <span class="badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25 rounded-pill px-2 py-1 shadow-sm" title="Partita Privata (Solo Amici)">
                ...
                <?php if(($p['status'] ?? '') === 'full' || ($p['posti_occupati'] ?? 0) >= $p['max_players']): ?>
                    <span class="badge bg-success rounded-pill shadow-sm py-1 px-2"><i class="bi bi-check-circle-fill me-1"></i>Completa</span>
                <?php elseif(($p['posti_occupati'] ?? 0) == $p['max_players'] - 1): ?>
                    <span class="badge bg-danger rounded-pill shadow-sm py-1 px-2 animate__animated animate__pulse animate__infinite">🔥 Urgente: Manca 1!</span>
                <?php else: ?>
            ...
            <h5 class="card-title fw-bold text-center text-white mb-1 d-flex align-items-center justify-content-center gap-1">
            ...
            <p class="text-center text-muted small mb-2"><i class="bi bi-person-fill text-secondary"></i> Org: <?= e($p['host_name'] ?? '') ?></p>

            <!-- Barra di Progresso Iscritti -->
            <div class="my-2 px-1">
                <div class="progress rounded-pill shadow-sm" style="height: 6px;" title="Iscritti: <?= $p['posti_occupati'] ?? 0 ?>/<?= $p['max_players'] ?> (<?= $pct ?>%)">
            ...
            <div class="d-flex justify-content-between align-items-end mt-auto pt-2">
                <div>
                    ...
                    <div class="fw-bold fs-5 text-body">
                        <span class="<?= ($p['posti_occupati'] ?? 0) >= $p['max_players'] ? 'text-success' : 'text-primary' ?>">
                            <?= $p['posti_occupati'] ?? 0 ?>
                        </span>
            ...
            <div class="mt-3 pt-3 border-top border-opacity-10 d-block w-100" onclick="event.stopPropagation();">
                <?php if(($p['status'] ?? '') === 'cancelled'): ?>
                    <div class="w-100 text-center py-2 bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 rounded-pill fw-bold small">
                ...
                <?php else: ?>
                    ...
                    <?php if($userRegStatus): ?>
                        <?php if($userRegStatus === 'registered'): ?>
                            <div class="w-100 text-center py-2 bg-success bg-opacity-10 text-success border border-success border-opacity-25 rounded-pill fw-bold small" style="font-size: 0.75rem;">
                                <i class="bi bi-check-circle-fill me-1"></i> Iscritto
                            </div>
                        <?php else: ?>
                            <div class="w-100 text-center py-2 bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25 rounded-pill fw-bold small" style="font-size: 0.75rem;">
                    ...
                    <?php else: ?>
                        <?php if(($p['posti_occupati'] ?? 0) >= $p['max_players'] || ($p['status'] ?? '') === 'full'): ?>
                            <form action="<?= url('/matches/' . $p['id'] . '/join') ?>" method="POST" class="m-0 w-100">
                                ...
                                <button type="submit" class="btn btn-sm btn-outline-warning rounded-pill w-100 py-2 shadow-sm fw-bold">
                        ...
                        <?php else: ?>
                            <form action="<?= url('/matches/' . $p['id'] . '/join') ?>" method="POST" class="m-0 w-100">
                                <input type="hidden" name="csrf_token" value="<?= e($_SESSION['csrf_token'] ?? '') ?>">
                                <button type="submit" class="btn btn-sm btn-primary rounded-pill w-100 py-2 shadow-sm fw-bold">Unisciti</button>
                            </form>
                        <?php endif; ?>
                    <?php endif; ?>
                <?php endif; ?>

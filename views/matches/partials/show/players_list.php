...
?>
<!-- Lista Iscritti -->
<h2 class="fw-bolder mb-3 mt-5 px-2 fs-4">Iscritti (<span class="text-primary"><?= $occupied_seats ?></span> / <?= $match['max_players'] ?>)</h2>
<div class="list-group shadow-sm rounded-4 border-0 mb-5" role="list" aria-label="Lista dei giocatori iscritti">
    <?php if (!empty($registrations)): ?>
        <?php foreach ($registrations as $reg): 
            ...
            ?>
            <div class="list-group-item border-0 border-bottom d-flex justify-content-between align-items-center py-3 bg-body <?= $reg['status'] === 'waitlist' ? 'opacity-75' : '' ?> hover-scale transition-all" role="listitem">
                <div class="d-flex align-items-center">
                    <div class="bg-<?= $reg['team'] === 'home' ? 'danger' : ($reg['team'] === 'away' ? 'primary' : 'primary') ?> text-white rounded-circle d-flex justify-content-center align-items-center me-3 fs-5 fw-bold shadow-sm border border-2 border-white match-show-avatar"
                    ...
                    <div>
                        <h3 class="mb-0 fw-bold d-flex align-items-center gap-2 flex-wrap fs-6">
                            ...
                            <?php if($reg['has_guest']): ?>
                                <span class="badge bg-info text-dark shadow-sm">+1 Ospite</span>
                            <?php endif; ?>
                            <?php if($reg['status'] === 'waitlist'): ?>
